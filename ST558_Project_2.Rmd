...
```{r}
#pop.life.scale <- select(pop.life, is.numeric) %>% mutate_all(~(scale(.) %>% as.vector)) %>% cbind(select(pop.life, is.factor))

#Scatterplot for n_tokens_content v.s. Shares.
scatter <- ggplot(data = pop.life, aes(x = n_tokens_content, y = shares))
...
train.index <- createDataPartition(y = pop.life$shares, p = 0.7, list = F)
train <- pop.life[train.index, ] # training set
test <- pop.life[-train.index, ] # test set
```
