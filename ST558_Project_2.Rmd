...
Supervised learning methods that will be used in this project include: linear regression, generalized linear model, logistic regression, random forest regression, random forest classification, boosted tree method, or any other method that we will find that could be applicable through our discovering of the data.  
...
Some tables containing grouped by channel and weekday
```{r}
#summary statistics 
...
# using the data set containing weekday info in one column. 
table(X$weekday)
X %>% 
    group_by( weekday ) %>% 
...
```
  ...
  This is a boxplot for different channels
```{r}  
g<-ggplot(X,aes(x = channel,
                       y = shares))
  g + 
...
pred.lm <- predict(lm.fit3, newdata = test.sub)
test.MSE.lm <- mean((pred.lm - test.sub$shares)^2)
```
...
test.MSE.boost <- mean((pred - test.sub$shares)^2)
